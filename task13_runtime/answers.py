...
NOTES.update(metric_not_computable='当前已查询范围的M03分母为0，比例不可计算（null）。',
             metric_data_error='当前输入数据未通过校验，无法提供可靠指标。')
...
def verify_answer(text,numeric,knowledge,clusters,rules,turn_id):
    ...
    for block in [s.strip() for s in v['answer_markdown'].splitlines() if s.strip()]:
        ...
        if key.startswith('metric_'):note_evidence[key]=numeric.metric_note(key,turn_id)

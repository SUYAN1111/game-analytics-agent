"""Three disjoint evidence families; host owns all labels, scopes, units and prose."""
import re
...
NOTES={**OLD_NOTES,'segmentation_unavailable':'本轮冻结分群不可用，不能提供群画像。',
...
def completion_summary(result):
    ...
    summary['complete_reference_blocks']=len(re.findall(r'\{\{(?:claim|knowledge|cluster|note):[^{}\n]+\}\}',text)) if isinstance(text,str) else None
    return summary
...
def require_evidence_families(value,numeric,knowledge,clusters):
    """Classify only actual registered evidence IDs, never words in free prose."""
    registries={'claims':numeric.entries,'knowledge_selections':knowledge.entries,'cluster_selections':clusters.entries}
...
def answer_envelope(text):
    ...
    v=parse_json(text)
    require(type(v) is dict and set(v)=={'answer_markdown','claims','knowledge_selections','cluster_selections'},'answer','exactly four answer fields required')
    require(type(v['answer_markdown']) is str and len(v['answer_markdown'])<=1024 and
            all(type(v[k]) is list for k in ('claims','knowledge_selections','cluster_selections')),'answer','invalid body/selection types')
    require(len(v['claims'])+len(v['cluster_selections'])<=12 and len(v['knowledge_selections'])<=8,'answer_limits','combined numeric cap 12, knowledge cap 8')
    ids=set()
    for key,prefix,fields,limit in [('claims','c',{'id','evidence_id','field','scope'},12),
        ('cluster_selections','s',{'id','evidence_id','field','scope'},12),('knowledge_selections','k',{'id','evidence_id','chunk_id'},8)]:
        for s in v[key]:
    ...
    for block in blocks:
        m=re.fullmatch(r'\{\{(claim|knowledge|cluster):([cks][1-9]\d?)\}\}',block)
        n=re.fullmatch(r'\{\{note:([a-z_]+)\}\}',block)
        if m:
            require(m[2][0]=={'claim':'c','knowledge':'k','cluster':'s'}[m[1]] and m[2] in ids,'answer_body','wrong namespace/reference');used.append(m[2])
...
def verify_answer(text,numeric,knowledge,clusters,turn_id):
    v=answer_envelope(text)
    require_evidence_families(v,numeric,knowledge,clusters)
    ...
    mapping.update({r['selection']['id']:r['rendered_fact'] for r in groups});rendered=[]
    ...
    return {'answer_markdown':'\n\n'.join(rendered),'claims':facts,'knowledge_selections':quotes,'cluster_selections':groups,
            'status':'reference_checked_candidate','presentation_contract':'host_bound_three_evidence_v1'}

...
| Linux / Neon / Vercel | 已有版本的 Linux 自动验收通过，Neon 初始化完成，Vercel 已部署。公网日志确认临时磁盘写满；已改为紧凑索引，本地全部玩家数据核对、五类工具及重复统计通过。新增 512 MiB 磁盘限制的 Linux 验收与公网更新后实测待完成 | [云端排查记录](docs/CLOUD_VALIDATION_2026-09-18.md) |

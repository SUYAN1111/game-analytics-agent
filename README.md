...
| Linux / Neon / Vercel | 已有版本的 Linux 自动验收通过，Neon 初始化完成，Vercel 已部署。公网日志确认打包遗漏分析资产；归档修复通过 15 项本地打包检查及五类真实工具分析，待推送及公网验收 | [云端排查记录](docs/CLOUD_VALIDATION_2026-09-18.md) |
...
架构为 **Vercel 承载网页与分析后台，Neon PostgreSQL 保存对话、证据和费用账本**。网页已部署；分析资产打包修复已通过本地验证，完整公网分析仍待部署新版后验收。

...
| Linux / Neon / Vercel | 提交 `34a8051` 的 Linux 自动验收通过，Neon 初始化完成；Vercel 已进入函数打包，774.37 MB 包体被标准 500 MB 限制拦截。配置已显式启用 Fluid compute 与 Large Functions，待云端重试，尚未上线 | [云端排查记录](docs/CLOUD_VALIDATION_2026-09-18.md) |
...
目标架构是 **Vercel 承载网页与分析后台，Neon PostgreSQL 保存对话、证据和费用账本**。适配代码已加入仓库，目前还没有经过实际 Vercel 部署验收。

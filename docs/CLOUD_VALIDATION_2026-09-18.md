...
## 受限 Linux 验收发现跨任务缓存残留

[GitHub Actions 35329945511](https://github.com/SUYAN1111/game-analytics-agent/actions/runs/35329945511)，提交 `d857dbc`：构建、资产归档、隔离依赖、全量索引对照、HTTP、公开访问、诊断和清理检查通过；512 MiB tmpfs 中前五类分析通过，第六项 `compare_again` 失败。用户提供的完整报告显示任务 `20666bce778f4d3985bf67e68dfe0522` 在索引约 500,000 条会话后报 SQLite 磁盘满，随后预算镜像、错误日志以及测试报告也无法写入。Node.js Action 弃用提示仍只是警告。

检查固定版本 DSH 单文件运行程序的实际加载代码与本项目环境传递链发现：运行程序会将本机库解压到 `PKG_NATIVE_CACHE_PATH/pkg/…`，未指定时使用用户主目录下的 `.cache/pkg/…`。原 `clean_environment()` 在每次子进程边界重新使用 `STATE/temporary/<PID>`，Linux 同时将 `HOME` 指向那里；云端完成任务时只删除 `STATE/cloud-<执行标识>`，这些目录不在删除范围内。Windows 原测试没有改变主目录，因而也没有把这类缓存纳入任务磁盘占用验证。已确认这处生命周期缺口；原失败报告没有记录各遗留目录大小，不声称取得了 Linux 缓存的逐文件占用。

修复由 Host 为每次执行指定任务内的 `session/temporary`，Python 驱动、DSH 插件、桥接、MCP 和下游工具继承同一目录。显式设置临时目录、本机库缓存和 XDG 缓存位置；Windows 也明确设置本机库缓存，便于本地真实覆盖。路径必须位于状态根目录内，不能使用状态根目录本身。模型和数据库凭据仍不透传给工具子进程。只有原有进程退出和账本处理确认成功后，云端才删除整个任务目录；没有放宽未确认执行的清理规则、抹掉未知费用或自动重试分析。

新增 `scripts/check_runtime_temporary.py`：三个真正嵌套的子进程验证临时目录继承、缓存归属、凭据隔离和非法根目录拒绝，3 组检查通过。六次真实 DSH/MCP 离线分析再次全部成功，证据数为 **7、2、5、10、3、7**，私有诊断为空；每次结束后的本机库缓存均为 **0**，资产和测试自有文件占用均回到 **237,072,384 字节**，没有新增 PID 临时目录遗留。峰值文件占用为 **401,588,224 字节，约 383 MiB**，其中实际观测到的本机库缓存峰值为 **1,052,672 字节**。报告为 `state/cloud-cache-runtime/cloud-runtime-results.json`，`assets_from_archive=true`、`paid_calls=0`。资产归档 16 项、发布完整性 18 项、清理失败 7 项及原启动失败报告检查继续通过。

Linux CI 保留真实 512 MiB tmpfs，并新增每次任务完成后检查文件系统实际占用是否回落至基线（容差 1 MiB），覆盖已删除但仍被进程打开的文件。报告通过 `CLOUD_TEST_REPORT_PATH` 写到受限卷之外，每完成一项就保存，避免磁盘满再次丢失报告。以上数值来自 Windows 本地测试；受限 Linux 与真实 Vercel/DeepSeek 尚待用户推送后验证。本轮没有提交、推送、部署或修改生产数据库、资产附件及环境变量。
